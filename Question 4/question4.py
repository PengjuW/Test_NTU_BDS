...
datatxt = np.loadtxt('input_question_4')
datatxt = np.array(datatxt,dtype=int)

sr, er, c, labels, rowEquivalences, colEquivalences = bwlabel1(datatxt)
# print(sr, er, c, labels)
k = 0
fop = open('output_question_4.txt','ab')
for i in range(len(datatxt)):
    fl= 0
    for j in range(len(datatxt[i])):
        if datatxt[i][j]==1:
            fl=1
            fop.write(str(str(sr[k])+' ').encode())
        else:
            fop.write(str(str(datatxt[i][j]) + ' ').encode())
    if fl:
        k = k + 1
    fop.write(str('\n').encode())
